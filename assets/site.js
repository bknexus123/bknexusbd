...
if (projectForm) {
  projectForm.addEventListener('submit', async (event) => {
    ...
    data.set('_subject', `New BK Nexus Project Inquiry — ${data.get('company') || data.get('name')} — ${data.get('country')}`);
    ...
    try {
      ...
      status.textContent = 'Thank you. Your project inquiry has been submitted successfully. Our team will contact you soon.';
    } catch (error) {
      status.className = 'form-status is-visible is-error';
      status.textContent = error.name === 'AbortError'
        ? 'The request took too long. Please check your connection and try again.'
        : 'We could not submit your inquiry right now. Please try again or email info@bknexusbd.com.';
    } finally {
